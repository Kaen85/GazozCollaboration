      ...
      <div className="max-w-7xl mx-auto w-full h-full flex flex-col">
        ...
        <div className="flex-none flex justify-between items-center mb-4">
          ...
          <button 
            ...
            className="bg-primary hover:bg-primary-hover text-white font-bold py-2.5 px-6 rounded-xl shadow-lg transition-all transform hover:scale-105 active:scale-95 flex items-center text-sm border-none cursor-pointer"
          >
        ...
        {/* SEARCH AND FILTERS */}
        <div className="flex-none grid grid-cols-1 md:grid-cols-4 gap-3 bg-surface p-2 rounded-2xl border border-border shadow-sm mb-4">
          ...
          <select 
            ...
            className="w-full bg-app border border-border text-text-main text-base rounded-xl px-4 py-2 outline-none focus:border-primary cursor-pointer appearance-none"
          >
        ...
        <div className="flex-grow overflow-y-auto pr-1 custom-scrollbar">
          {loading ? (
            ...
            <div className="p-10 text-center bg-surface/20 rounded-2xl border border-dashed border-border">
              ...
              {currentProjects.map(project => {
                ...
                return (
                  <Link to={`/project/${project.id}`} key={project.id} className="relative group h-fit">
                    <div className="relative p-4 rounded-2xl bg-surface transition-all duration-300 flex flex-col justify-between hover:shadow-xl shadow-sm border border-border hover:border-primary/50">
                      ...
                      <div>
                        <div className="flex justify-between items-start mb-2">
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider ${isOwner ? 'bg-primary/10 text-primary' : 'bg-app text-text-secondary border border-border'}`}>{isOwner ? 'Owner' : 'Member'}</span>
                          <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold border transition-colors ${project.is_public ? 'text-emerald-600 border-emerald-500/20 bg-emerald-500/5' : 'text-amber-600 border-amber-500/20 bg-amber-500/5'}`}>
                      ...
                      <div className="mt-4 pt-3 border-t border-border flex justify-between items-center text-[10px] uppercase tracking-tight">
        ...
        <div className="flex-none pt-2 pb-4">
          <div className="flex justify-between items-center bg-surface p-3 border border-border rounded-xl shadow-sm">
            <span className="text-xs text-text-secondary font-bold uppercase tracking-widest opacity-80">
              {filteredProjects.length > 0 ? <>Showing <span className="text-text-main font-black">{((currentPage - 1) * projectsPerPage) + 1} - {Math.min(currentPage * projectsPerPage, filteredProjects.length)}</span> / {filteredProjects.length}</> : "Showing 0 / 0"}
            </span>
            <div className="flex items-center gap-3">
              <button onClick={() => paginate(currentPage - 1)} disabled={currentPage === 1} className="p-2 rounded-lg hover:bg-app disabled:opacity-30 transition-all border border-border text-text-secondary hover:text-text-main"><FiChevronLeft size={20} /></button>
              <button onClick={() => paginate(currentPage + 1)} disabled={currentPage === totalPages} className="p-2 rounded-lg hover:bg-app disabled:opacity-30 transition-all border border-border text-text-secondary hover:text-text-main"><FiChevronRight size={20} /></button>
            </div>
          </div>
        </div>
      </div>
